...
#include "../Chunks/hsv.cginc"
 ...
 UNITY_INITIALIZE_OUTPUT( Input , data );
        ...
        float3 fTan = _transferBuffer[id].tangent;
        float2 fUV  = _transferBuffer[id].uv;
            ...
            v.tangent = float4(fTan,0);//float4( normalize(cross(fNor,float3(0,1,0))),1);
      ...
      void surf (Input IN, inout SurfaceOutputStandard o) {
        ...
        float3 nor = UnpackNormal (tex2D (_BumpMap, IN.texcoord1.xy * float2( 1 , 1)));
          ...
          o.Albedo =  1;//(nor * .5 + .5) * mainCol * hsv( length(mainCol) * .3 + _Time.x * 5 + IN.debug,1,1);// 1000*(UnpackNormal (tex2D (_BumpMap, IN.texcoord1.xy * float2( 1 , 1)))-float3(0,0,1));// * .5 + .5; //tex2D (_MainTex, IN.texcoord1.xy).rgb;
         o.Smoothness = 1.3;//0.8f;
         o.Metallic = .8;//.9f;
          o.Normal = nor;
      }
      ENDCG
